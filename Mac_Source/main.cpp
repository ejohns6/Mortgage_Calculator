...
int main(int argc, char *argv[])
{
    ...
    QFile I_AM_HERE("I_AM_HERE.txt");
    I_AM_HERE.open(QIODevice::WriteOnly);
    I_AM_HERE.close();

    //Get the info in
    QFile InputData(QString(QCoreApplication::applicationDirPath().toUtf8().constData()) + "/../../../InputData.txt");
    if(!InputData.exists()){
        printf(QString(QDir::currentPath()).toUtf8().constData());
        printf("\nFAIL\n");
        return 1;
